...
public interface CreditPaymentRepository extends JpaRepository<CreditPayment, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT \n" +
            ...
            "WHERE cl.id = :clientId AND cr.closed_date is null\n" +
            "GROUP BY \n" +
            "    year, month\n" +
            "ORDER BY \n" +
            "    year, month ASC")
    List<Object[]> findDebtByMonths(Long clientId);
    ...
    @Query(nativeQuery = true, value = "SELECT \n" +
            ...
            "    EXTRACT(MONTH FROM cp.payment_date) AS month \n" +
            "FROM \n" +
            "    test.debit_account da\n" +
            "JOIN \n" +
            "    test.credit cr ON da.id = cr.debit_account_id\n" +
            "JOIN \n" +
            "    test.credit_payment cp ON cr.\"uuid\" = cp.credit_id \n" +
            "where da.id = :accountId\n" +
            "GROUP BY \n" +
            "    year, month\n" +
            "ORDER BY \n" +
            "    year, month ASC")
    List<Object[]> findDebtByMonthsWithAccountId(Long accountId);
}
